...
function AzimuthalAngleDistributionPlot(sol,species::Vector{String},PhaseSpace::PhaseSpaceStruct,type::Animated;theme=DiplodocusDark(),order::Int64=0,TimeUnits::Function=CodeToCodeUnitsTime,plot_limits=(nothing,nothing),wide=false,legend=true,framerate=12,filename="AzimuthalAngleDistribution.mp4",figure=nothing)
    ...
    for (species_idx, species_name) in enumerate(species) 
        ...
        if sum(@. !isnan(dNdphi) * !isinf(dNdphi) * !iszero(dNdphi)) == 1 # there is only one valid position so scatterlines doesn't work
            idx = findfirst(!iszero,dNdphi)
            lines!(ax,[mh[idx]/pi, mh[idx]/pi],[-20.0, log10(dNdphi[idx])],linewidth=2.0,color = color,linestyle=linestyles[species_idx])
        else
            scatterlines!(ax,mh./pi,log10.(dNdphi),linewidth=2.0,color = color,markersize=0.0,linestyle=linestyles[species_idx])
        end
