...
"""
function TimeScalePlot(method::DiplodocusTransport.SteppingMethodType,sol::DiplodocusTransport.OutputStruct,t_idxs::Vector{Int64};wide=false,paraperp::Bool=false,u_avg::Bool=true,logt::Bool=false,p_timescale=false,legend=true,dt::Bool=false,plot_limits=(nothing,nothing),theme=DiplodocusDark(),TimeUnits::Function=CodeToCodeUnitsTime,direction::String="all")
    ...
    for (idx,t_idx) in enumerate(t_idxs)
        ...
        t_idx_global = find_closest(tr,t)-1
        if t_idx_global != 1
